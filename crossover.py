...
#     """
#     Perform a crossover between two parents at a randomly selected crossover point.
#     Ensures entire instruments are maintained without splitting individual details.
#     """
#     # Check that both parents have the same structure
#     if len(parent1) != len(parent2):
#         raise ValueError("Both parents must have the same number of instruments")
    
#     # Choose a crossover point ensuring we preserve entire instruments
#     crossover_point = random.randint(1, len(parent1) - 1)
    
#     # Generate children by combining segments from both parents
...
#     # Initialize an empty list to store the generated children
#     children_data = []

#     # Process each pair of parents in the list
#     i = 0
#     while i < len(parents_data) - 1:
#         parent1, parent2 = parents_data[i], parents_data[i + 1]
#         child1, child2 = crossover(parent1, parent2)
#         children_data.extend([child1, child2])
#         i += 2
    
#     # If there’s an odd parent, pair it with the last child generated
#     if len(parents_data) % 2 != 0:
#         last_parent = parents_data[-1]
#         child1, child2 = crossover(last_parent, children_data[-1])  # Crossover with the last child
#         children_data.extend([child1, child2])

#     # Write the generated children data to the output JSON file
#     with open(output_file, 'w') as f:
#         json.dump(children_data, f, indent=4)

# # Example usage
# input_file = 'initial_population.json'  # Input file path
# output_file = 'children_population.json'  # Output file path
# generate_children_from_file(input_file, output_file)

# print(f"Children data has been written to {output_file}")
